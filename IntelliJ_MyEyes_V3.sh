...
curl -sL -o "$HOME/IntelliJ_MyEyes.sh" "https://raw.githubusercontent.com/arghya339/IntelliJ-MyEyes/refs/heads/main/IntelliJ_MyEyes_V3.sh"
...
pkgInstall "python"
pkgInstall "rust"
python3 -c "import bcrypt" 2>/dev/null && echo -e "$good bcrypt python module installed." || { echo -e "$running Installing bcrypt python module using pip by building wheel for bcrypt using rust..."; pip install bcrypt &>/dev/null; }
...
if su -c "[ -f /data/data/com.snapchat.android/databases/memories.db ]"; then
  ...
  echo -e "$running Brute forcing hash using Bcrypt.."
  [ "$(su -c 'getenforce 2>/dev/null')" = "Enforcing" ] && { su -c "setenforce 0"; writeSELinux=true; } || writeSELinux=false
  su -c "cmd deviceidle whitelist +com.termux" &> /dev/null
  [ $writeSELinux == true ] && su -c "setenforce 1"
  termux-wake-lock
  pincode=$(python3 -c "import bcrypt; h=open('$hashes').read().strip(); [print(f'{i:04d}') or exit() for i in range(10000) if bcrypt.checkpw(f'{i:04d}'.encode(), h.encode())]")
  termux-wake-unlock
  ...
  if [ ${#pincode} -eq 4 ]; then
    ...
    echo -e "$bad Failed to crack PinCode using Bcrypt!"
    # --- Prompt the user for input ---
...
echo -e "${Green}Powered by Bcrypt (github.com/pyca/bcrypt)"
termux-open-url "https://github.com/pyca/bcrypt"
